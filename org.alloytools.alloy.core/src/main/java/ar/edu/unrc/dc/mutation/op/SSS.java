...
public class SSS extends Mutator {
    ...
    private boolean strictTypeCheck() {
        Optional<Object> configValue = MutationConfiguration.getInstance().getConfigValue(MutationConfiguration.ConfigKey.OPERATOR_SSX_STRICT_TYPE_CHECK);
        return configValue.map(o -> (Boolean) o).orElseGet(() -> (Boolean) MutationConfiguration.ConfigKey.OPERATOR_SSX_STRICT_TYPE_CHECK.defaultValue());
    }
